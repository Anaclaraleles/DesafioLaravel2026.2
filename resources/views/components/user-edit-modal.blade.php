...
<div class="bg-[#f1f1f1] rounded-2xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto"
     @click.outside="editingUserId = null">
        ...
        <form action="{{ route('user.edit', $user) }}" method="post" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-2 gap-3 mb-4">
                ...
                <div>
                    ...
                    <input name="cpf" value="{{ old('cpf', $user->cpf) }}" class="w-full bg-white border border-gray-300 rounded-md px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#4E6E6E]"/>
                    @error('cpf')
                        <span class="text-xs text-red-500">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div class="flex justify-center gap-4 mt-8">
                <button type="button" @click="editingUserId = null" class="px-8 py-3 border-1 rounded-lg bg-[#C7D6D4] text-[#3a5555] font-medium hover:bg-[#b7c9c6] transition cursor-pointer shadow-lg shadow-[#4E6E6E]">Cancelar</button>
